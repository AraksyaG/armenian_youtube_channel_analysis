...
def parse_xml_file(file_path):
    try:
        ...
        channel_name = filename_parts[0]
        timestamp = filename_parts[-1].replace('.xml', '')
        ...
        return {
            ...
            'file_date': datetime.fromtimestamp(float(timestamp)).date(),
...
def upsert_channel(cursor, channel_data):
    try:
        # Insert or update channel based on original_channel_id
        cursor.execute("""
